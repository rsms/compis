...
// #define TRACE_PROMISE


#if defined(TRACE_PROMISE) && DEBUG
  #define trace(fmt, va...) _dlog(3, "spawn", __FILE__, __LINE__, fmt, ##va)
#else
  #undef TRACE_PROMISE
  ...
  uintptr a, uintptr b, uintptr c, uintptr d, uintptr e, uintptr f)
{
  ...
  assertf(g_parent_pid > 0, "not in parent promise_spawn");
