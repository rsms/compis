...
abuf_t is a string append buffer with fixed capacity for implementing snprintf-style
functions which writes to a limited buffer and separately keeps track of the number
of bytes that are appended independent of the buffer's limit.

Template for use with functions that uses abuf:
