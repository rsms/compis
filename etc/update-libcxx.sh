...
ARM_SOURCES=()
APPLE_SOURCES=()
pushd "$LIBUNWIND_DST/src" >/dev/null
for f in $(find . -type f -name '*.[csS]' -o -name '*.cpp'); do
  case "$f" in
    *EHABI*)       ARM_SOURCES+=( "$f" ) ;;
    *AppleExtras*) APPLE_SOURCES+=( "$f" ) ;;
    *AIXExtras*)   ;;
    *)             COMMON_SOURCES+=( "$f" ) ;;
  esac
done
